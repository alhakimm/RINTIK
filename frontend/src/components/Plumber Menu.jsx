        ...
        <div className='bg-gradient-to-b from-[#0A2236] to-[#15436E] w-full px-20 py-6 justify-between min-h-screen gap-2'>
            ...
            <div className="ml-4">
                ...
                <div className="space-y-5">
                    ...
                    {plumberList && plumberList.map((plumber) => (
                        <div className="p-4 pl-1 mb-5 border-b border-[#15436E] rounded-lg text-white hover:shadow-xl cursor-pointer grid grid-cols-4 grid-rows-2" key={plumber.id}>
                            <h2 className="text-blue-200 text-2xl transition duration-500 hover:font-bold hover:text-3xl col-span-2">{plumber.name}</h2>
                            <div className="flex items-center text-xs row-start-2 col-span-2">
                                <CgPin className="relative mr-1"/><p className="pr-1">{plumber.location} | {plumber.rating} </p><FaRegStar />
                            </div>
                            <p className="text-xs col-start-3 text-right pr-4 pt-1">from</p> <h2 className="col-start-4 text-3xl font-black">RM {plumber.price}</h2>
